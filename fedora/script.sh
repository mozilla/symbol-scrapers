...
function fetch_indexes() {
  ...
  echo "${URLS}" | while read url; do
    [ -z "${url}" ] && continue
    local regex="${url}/((releases|updates|development)/)?(testing/)?(test/)?(${release_regex}/)?(Everything/)?(${arch_regex}/)?((os|debug)/)?(tree/)?(Packages/)?(${package_folder_regex}/)?$"
...
function get_package_urls() {
  ...
  echo "${PACKAGES}" | grep -v '^$' | cut -d' ' -f1 | while read package; do
    grep -o "https\?://.*/${package}\(-debuginfo\)\?-[0-9].*\.fc..\.${arch_escaped_regex}\.rpm" all-packages.txt >> unfiltered-packages.txt
  done
}
